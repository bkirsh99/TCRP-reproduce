...
data_dic = out_dir + 'drug_feature/'

# Training settings
parser = argparse.ArgumentParser()

parser.add_argument('--dataset', type=str, default='GDSC', help='dataset to perform cross validation on')
parser.add_argument('--tissue', type=str, default='urinary_tract', help='Validation tissue, using the rest tissues for training')
parser.add_argument('--drug', type=str, default='Axitinib', help='Treated drug')
...
test_data_path = home_dir + '/data/fewshot_data/' + dataset + '/' + args.drug + '/' + args.tissue + '/'
...
base_run_name = args.run_name.split('_')[0]
base_line_outpath = home_dir + '/data/results/' + '{}/'.format(base_run_name) + 'TCRP_performances/' + args.drug + '/' + args.tissue + '/'
